...
class Cache implements CacheInterface
{
    ...
    public function get($cacheKey)
    {
		...
		// begin add by gen 2011/3/30 
		if (isset($_GET['dropcache']) && $_GET['dropcache']=='y')
		{
			if (true === $this->clear($cacheKey))
			{
				Debug::log('delete_an_existing_cache', $cacheKey);
			}
			else
			{
				Debug::log('try_to_delete_the_cache_but_did_not_succeed', $cacheKey);
			}
			return false;
		}
		// end
    ...
    {
    	$this->cache->$method($args[0],$args[1],$args[2]);
    }
}
?>